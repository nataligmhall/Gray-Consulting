...
(function initScramble() {
  const el = document.getElementById('navWordmark');
  if (!el) return;
  const FINAL = 'Gray';
  ...
  el.addEventListener('mouseenter', () => {
    ...
    function tick() {
      ...
      const out = FINAL.split('').map((ch, i) => {
        const revealAt = i * 0.2 + 0.1;
        if (t < revealAt) return CHARS[Math.floor(Math.random() * CHARS.length)];
        return ch;
      }).join('');
      el.textContent = out;
      frame++;
